...
// Firebase Configuration (with your specific API Key)
const firebaseConfig = {
  apiKey: "AIzaSyAUuY2mGhSFzeghdVeAhYGjLeu13PacvzM",
...
const WINE_QUIZ_QUESTIONS = [
  ...
  },
  {
    ...
    correctAnswer: "Ice Wine",
    explanation:
      "Ice wine is a sweet dessert wine that is best served very chilled to enhance its sweetness and acidity.",
  },
  {
    question:
...
const App = () => {
      ...
      const unsubscribe = onAuthStateChanged(auth, async (user) => {
        if (user) {
          ...
          const userProfileRef = doc(
            db,
            `artifacts/${firestoreAppId}/users/${user.uid}/profile`,
            "userProfile"
          );
  ...
  const handleSetName = async () => {
    ...
    try {
      const userProfileRef = doc(
        db,
        `artifacts/${firestoreAppId}/users/${userId}/profile`,
        "userProfile"
      );
      await setDoc(
        userProfileRef,
  ...
  const callGeminiAPI = async (prompt, schema = null) => {
    ...
    const apiKey = "AIzaSyCdBiyQp5YUW6SyoL6J_BXBzj7V-MN6V-Y"; // Your Gemini API Key
